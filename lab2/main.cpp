...
class board {
public:
	board(uint64_t raw = 0) : raw(raw) {} 			//create board obj, initial raw to variable raw
	board(const board& b) = default;				//this obj's alias is b
	board& operator =(const board& b) = default;	//use op = to copy this obj
	operator uint64_t() const { return raw; }		//use unit64_t() name = b.operator unit64t() to change type
...
class feature {
public:
	feature(size_t len) : length(len), weight(alloc(len)) {} //std::cout<< "->" << *(weight+len-1);
	feature(feature&& f) : length(f.length), weight(f.weight) { f.weight = nullptr; } //move constructor
...
public: // should be implemented
	...
	/**
	 * estimate the value of a given board, V(b)
	 */
	virtual float estimate(const board& b) const = 0;
	
	/**
	 * update the value of a given board, and return its updated value, V(b) = V(b) + alpha( r + V(s'') - V(s) )
	 */
...
protected:
	...
	size_t length;		//std::cout<< "->" << *(weight+(len)-1);
	float* weight;
};
...
public:
	...
	/**
	 * estimate the value of a given board V(b), with a specific feature, 
	 * compute all possible state s''  value given s,a
	 * V(s'') can be found in operator[](index), index = indexof(isomorphic[i], b);
	 */
...
public:
	...
	void dump(const board& b, std::ostream& out = info) const {
		for (int i = 0; i < iso_last; i++) {
			...
			size_t index = indexof(isomorphic[i], b); 			//get index of weight for binary representation
			for (size_t i = 0; i < isomorphic[i].size(); i++) { //transform each index to hex representation 
				out << std::hex << ((index >> (4 * i)) & 0x0f);
			}
			out << std::dec << ") = " << operator[](index) << std::endl;	// operator[](index) return weight[i];
		}
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {		//Is a valid move?
				...
				move->set_value((move->reward() + expect/count));
	 ...
	 */
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {	//unit
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("weight2.bin");

	// train the model
	std::vector<state> path;		
	path.reserve(20000); 		
	// train for 100,000 time
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
			...
			state best = tdl.select_best_move(b);	//perform argmax a
			path.push_back(best);

			if (best.is_valid()) { 					//invalid action (cause after == before or score == -1)
				debug << "best " << best;
				score += best.reward();
				b = best.after_state();
				b.popup();
		...
		s_score = s_score + score;
		if (n%1000==0){
			scores.push_back(s_score/1000);
			s_score = 0;
		}
	}

	// store the model into file
	tdl.save("weight2.bin");
	std::ofstream file;
    file.open("score2.csv");
    for(int s:scores){
        file<<s<<'\n';
    }
    file.close();
